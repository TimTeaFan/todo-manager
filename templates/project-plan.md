# Project Plan: {{TITLE}}

**ID**: {{ID}}
**Created**: {{DATE}}

## Goal

<!-- Describe the overall goal of this project in 1-3 sentences. -->
<!-- What is the expected end result? What deliverables should exist when done? -->

## Subtasks

<!-- Break the project into 3-10 subtasks. -->
<!-- Each subtask must be completable in ONE session. -->
<!-- Each subtask must produce a concrete deliverable (file, dataset, summary). -->
<!-- Order by dependency: tasks that others depend on come first. -->

| # | Subtask | Status | Deliverable | Depends on |
|---|---------|--------|-------------|------------|
| 1 | | pending | | — |
| 2 | | pending | | — |
| 3 | | pending | | — |

**Status values**: pending, in_progress, done, blocked, cancelled

## Blockers

<!-- List any blockers that prevent progress. -->
<!-- Format: [YYYY-MM-DD] Description of blocker -->

## Notes

<!-- Any additional context, links, or decisions. -->
